	...
	var BattleRoom = this.BattleRoom = ConsoleRoom.extend({
		...
		showTooltip: function(thing, type, elem, ownHeight, isActive) {
			...
			case 'move':
				...
				// Show the Same Type Attack Bonus if necessary.
				if (basePower !== '&mdash;' && myPokemon[this.choice.choices.length].types.indexOf(moveType) > -1) additionalInfo = '<p>Has STAB.</p>';
		...
		getMoveType: function(move, pokemon) {
			...
			if (move.id === 'naturepower') {
				...
				if (this.battle.hasPseudoWeather('Grassy Terrain')) moveType = 'Grass';
				if (this.battle.hasPseudoWeather('Misty Terrain')) moveType = 'Fairy';
			}
		...
		getMoveBasePower: function(move, pokemon, target) {
			...
			if (move.id === 'electroball') {
				...
				var min = 0;
				var max = 0;
				if (target.volatiles.formechange) template = Tools.getTemplate(target.volatiles.formechange[2]);
			...
			if (target.weightkg) {
				...
				if (move.id === 'lowkick' || move.id === 'grassknot') {
					...
					if (target.volatiles.autotomize) basePowerComment = ' (Approximation)';
				}
